...
In case no column name is provided through the `cols` argument, the first numeric column is taken by default. Column names for the `cols` argument can be requested from `DTSg` objects with the help of the `cols` method by the way. It supports a `class` and/or `pattern` argument:
