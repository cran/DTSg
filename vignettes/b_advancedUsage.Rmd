...
An extension to reference semantics of existing `DTSg` objects is reference semantics during object creation. This behaviour can be triggered with the help of the `swallow` argument of the `new` method. If set to `TRUE`, a `data.table` provided through the `values` argument is "swallowed" by the `DTSg` object, i.e., no copy of it is made and all references to it are removed from the global environment upon successful object creation:
...
The opposite of swallowing is called dropping. This term refers to querying the *values* of a `DTSg` object as a reference while removing all references to the original `DTSg` object from the global environment at the same time:
