...
In order to get mean monthly river flows as an example, we can use the `aggregate` method with one of the package's temporal aggregation level functions as its `funby` argument (the `fun` argument also accepts a named character vector or `list` of functions which allows for calculating several summary statistics at once):
