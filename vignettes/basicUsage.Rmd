...
Now that we have a data set, we can create our first object by providing it to the `new` method of the package's main class generator `DTSg`. In addition, we specify an ID in order to give the new object a name:
...
In case no column name is provided through the `cols` argument, the first numeric column is taken by default. Column names for the `cols` argument can be requested with the help of the `cols` method by the way. It supports a `class` argument:
